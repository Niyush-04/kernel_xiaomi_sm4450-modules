...
static const struct reg_map_info reg_map_info_table[] = {
	...
	[CHIP_TYPE_SIA9196] = {
		.chip_type = CHIP_TYPE_SIA9196,
		.reg_addr_width = 8,
		.reg_val_width = 16,
		.chip_id_addr = 0x06,
		.chip_id_ranges = {{0x6190, 0x6191},{0x6200, 0x6200}},
		.chip_id_range_num = 2
	},
	[CHIP_TYPE_SIA9177] = {
	...
	[CHIP_TYPE_SIA9187] = {
		.chip_type = CHIP_TYPE_SIA9187,
		.reg_addr_width = 8,
		.reg_val_width = 16,
		.chip_id_addr = 0x06,
		.chip_id_ranges = {{0x5d80, 0x5d90}},
		.chip_id_range_num = 1
	},
	[CHIP_TYPE_SIA5118] = {
		.chip_type = CHIP_TYPE_SIA5118,
		.reg_addr_width = 8,
		.reg_val_width = 16,
		.chip_id_addr = 0x00,
		.chip_id_ranges = {{0x3B00, 0x3B00}},
		.chip_id_range_num = 1
	},
};
	...
	SIPA_REG_PROC *reg, unsigned int val_width)
{
	...
	case SIPA_REG_READ:
		...
		if (0 != reg->delay)
			usleep_range(reg->delay, reg->delay);
		
		//support print status val
		//if (reg->addr <= 0x14)
	...
	case SIPA_REG_CHECK:
		...
		if ((val & reg->mask) != (reg->val[si_pa->scene] & reg->mask)) {
			pr_err("[  err][%s] %s: addr:0x%x, val:0x%x, expected:%x, mask:0x%x\n", LOG_FLAG, __func__,
				reg->addr, val, reg->val[si_pa->scene], reg->mask);
			return -1;
		}
	...
	sipa_dev_t *si_pa)
{
	...
	if ((val & reg_en->mask) == (reg_en->val & reg_en->mask))
		return true;
	pr_info("[ info][%s] %s: addr:0x%x val:0x%x mask:%x  fw_val:%x  \r\n",
		LOG_FLAG, __func__, reg_en->addr, val, reg_en->mask, reg_en->val);
	return false;
}
