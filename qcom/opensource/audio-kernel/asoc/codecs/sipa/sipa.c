	...
	char *buf)
{
	...
	tb = (char *)kmalloc(2048, GFP_KERNEL);
	if (tb == NULL) {
		pr_err("[  err][%s] %s: kmalloc fail !!! \r\n",
					LOG_FLAG, __func__);
		return -1;
	}
...
#if IS_ENABLED(CONFIG_LCT_AUDIO_INFO)
	if(!(si_pa->chip_type < CHIP_TYPE_SIA8101 || si_pa->chip_type >= CHIP_TYPE_UNKNOWN)) {
		strcpy(lct_audio_pa_info, support_chip_type_name_table[si_pa->chip_type]);
		strcat(lct_audio_pa_info, "-SIPA");
		lct_audio_info_set_pa_name(lct_audio_pa_info, strlen(lct_audio_pa_info));
	}
#endif
