...
static int fsm_check_dev_type(fsm_dev_t *fsm_dev)
{
	...
	if (fsm_dev->is1820 || fsm_dev->is1599) {
...
int fsm_get_rstrim(fsm_dev_t *fsm_dev)
{
	...
	if (fsm_dev->is1958 || fsm_dev->is1599) {
		fsm_dev->rstrim = 0;
		return 0;
	} else if (fsm_dev->is1820) {
		reg_rstrim = 0x07;
...
int fsm_probe(fsm_dev_t *fsm_dev, int addr)
{
	...
	fsm_dev->addr = addr;
	do {
#if defined(CONFIG_FSM_FS1815) || defined(CONFIG_FSM_FS1758) || defined(CONFIG_FSM_FS1599)
...
void fsm_get_livedata(struct fsm_livedata *lvdata)
{
	...
	for (dev = 0; dev < lvdata->ndev; dev++) {
		...
		if (fsm_dev == NULL) {
			pr_debug("invalid fsm_dev:%d", dev);
			continue;
		}
		index = fsm_get_index_by_position(fsm_dev->pos_mask);
		if (index < 0) {
			pr_debug("invalid index:%d", index);
			continue;
		}
